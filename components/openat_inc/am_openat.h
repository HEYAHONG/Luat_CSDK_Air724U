...
UINT32 OPENAT_write_i2c(                                        /* 实际写入长度 */
                            E_AMOPENAT_I2C_PORT port,          /* I2C 编号 */
                            UINT8 salveAddr,
                            CONST UINT16 *pRegAddr,              /* I2C外设寄存器地址 */
...
/*+\bug6390\liuchacheng\2022.3.18\增加ADC快速读取接口*/
...
/*-\NEW\shenyuanyuan\2019.4.19\开发AT+TRANSDATA命令*/
/*+\BUG\zxf\2021.9.17\BUG_4930:实现rtos.get_fatal_info和rtos.remove_fatal_info两个功能*/
...
/*-\NEW\liyaoyao\2021.07.06\添加Base64的加解密接口*/
/**Base64算法加密
*@note: 
*@param 	   outputData：存储加密后的数据
*@param 	   outputLenMax：存储加密后的数据的缓冲区的最大空间
*@param 	   outputLen：存储加密后数据的长度
*@param        inputData：存储待加密的数据
*@param        inputLen：待加密数据的长度
*@return	成功或失败的原因
**/
...
/**Base64算法解密
*@note: 
*@param 	   outputData：存储解密后的数据
*@param 	   outputLenMax：存储解密后的数据的缓冲区的最大空间
*@param 	   outputLen：存储解密后数据的长度
*@param        inputData：存储待解密的数据
*@param        inputLen：待解密数据的长度
*@return	成功或失败的原因
**/
